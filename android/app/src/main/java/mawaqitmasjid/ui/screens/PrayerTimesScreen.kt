// Auto-generated Compose screen for "Prayer Times" (F001)
...
fun PrayerTimesScreen(modifier: Modifier = Modifier) {
    LazyColumn(
        ...
        when (val state = uiState) {
            is PrayertimesUiState.Loading -> item { CircularProgressIndicator() }
            is PrayertimesUiState.Error -> item { Text(state.message, color = MaterialTheme.colorScheme.error) }
            is PrayertimesUiState.Content -> items(state.items) { item ->
                ListItem(
